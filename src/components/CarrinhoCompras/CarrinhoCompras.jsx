...
export function CarrinhoCompras() {
    ...
    const { attItem, delApenasUmItem, delPedido, delItens } = useContext(ContextSacolaCompras);
    ...
    const diminuirItens = (index) => {
        ...
        if (updateCanvasDados.items[index].quantidade > 1) {
            ...
            handleShow(updateCanvasDados.items[index].id, updateCanvasDados.id);
        }
    };


    const deletar = async (itemId, pedidoId) => {
        if (canvasDados.items.length === 1) {
            delPedido(pedidoId);
            delItens(pedidoId);
        ...
        const aumentarItens = async (index) => {
            ...
            const novaQuantidade = quantidade === null || quantidade === undefined ? 1 : quantidade + 1;
        ...
        function onSubmit(data) {
            const {
                status,
                metodoPagamento,
                endereco: { rua, cep, numero, complemento },
            } = data;
            ...
            <div>
                <Modal show={showModalCanvas} onHide={handleClose}>
                    ...
                    <Modal.Footer>
                        ...
                        <Button
                            variant="secondary"
                            onClick={() => deletar(itemId, pedidoId)}
                        >
                            Exluir item
                        </Button>
                    </Modal.Footer>
                ...
                <div className=" display-block">
                    <Button
                        ...
                        onClick={handleClick}
                    >
                        <i class="bi bi-cart icone-carrinho-compras text-light" icone-carrinho-compras></i>
                    </Button>
                </div>
                <div>
                    <Offcanvas
                                ...
                                <Offcanvas.Body>
                                    ...
                                    {canvasDados.items.map((item, index) => (
                                        <div className="preco-nome-canvas" key={item.id}>
                                            ...
                                            <button className="trash-canvas" style={{ cursor: 'pointer' }} onClick={() => deletar(item.id, canvasDados.id)}>
                                    ...
                                    <Form onSubmit={handleSubmit(onSubmit)}>
                                        ...
                                        <Form.Group className="mb-3">
                                            <Form.Label className="d-flex mt-3">Método de Pagamento</Form.Label>
                                            <Form.Select
                                        ...
                                        <div className="d-grid gap-2 col-12 flex-fill">
                                            <Button type="submit" className="white button_cadastro mt-5">
                                                Finalizar Pedido
                                            </Button>
                                        </div>
                                    </Form>
